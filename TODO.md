...
- [x] Update the chunking pipeline to delete files if the md does not exist, same for PDF. Implemented as layered sources of truth: PDFs are the source of truth for the `.md` files the PDF pipeline generates (marked `converted_from_pdf` in front matter; hand-authored Markdown is never deleted), and the set of `.md` files on disk is the source of truth for the vector store, the `memory:library` index, and chunking state.
  - [x] Reconciliation runs at the end of each pipeline's scan cycle with a two-phase tombstone: a missing file is first marked `Missing` with a timestamp, and deletion happens only when a later scan still finds it missing after `LIBRARY_RECONCILIATION_GRACE_SECONDS` (default 60s). Entries currently `Converting`/`Chunking` are skipped, which handles the mid-flight race. Collections/tables left empty are dropped so removed or renamed shelves don't linger. Black-box tests: `test_agents/library_reconciliation` @ default and @ no_qdrant.
  - [ ] Reconciliation is driven by pipeline state entries; vector-store points with no corresponding state entry (e.g. Redis wiped while Qdrant kept its data) are not cleaned up automatically — a full re-index covers that case.
