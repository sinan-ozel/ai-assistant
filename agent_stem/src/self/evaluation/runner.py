...
def validate_expectation(
    actual: Union[str, dict, int, float, None],
    ...
    try:
        ...
        if exp_type == "contains":
            ...
            if not isinstance(expected_values, list):
                return {
                    "passed": False,
                    "error": f"oneOf expectation requires a list of values, got {type(expected_values)}"
                }
            matched = False
            errors = []
            ...
            if not matched:
                return {
                    "passed": False,
                    "error": f"Response '{actual}' does not match any of {expected_values}. Errors: {errors}"
                }
            ...
            if not re.search(expectation["value"], str(actual)):
                return {
                    "passed": False,
                    "error": f"Response '{actual}' does not match regex '{expectation['value']}'"
                }

        elif exp_type in {"in_range", "range", "within_range"}:
            expect_in_range(actual, expectation)

        elif exp_type in {"approx_pct", "approximate_percentage", "percent_error", "within_percentage"}:
...
def run_evaluation_case(
    ...
    logger.info(f"Running evaluation case: {test_id} (repeat={repeat}, threshold={threshold})")
        ...
        if workflow_path:
            ...
            with Memory() as memory:
                if hasattr(memory, 'workflow_evaluation_state'):
                    state = memory.workflow_evaluation_state.get(workflow_path, {})
                    if state.get("cancelled"):
                        logger.info(f"Evaluation cancelled for workflow: {workflow_path}")
        ...
        try:
            for step_idx, step in enumerate(steps):
                ...
                if not content.strip() and raw_message.get('tool_calls'):
                    try:
                        content = raw_message['tool_calls'][0]['function']['arguments']
                        logger.info(f"Extracted content from tool_calls for case={test_id}")
                ...
                for expectation in expectations:
                    if isinstance(expectation.get("value"), dict) or expectation.get("type") == "oneOf":
                        try:
                ...
                step_passed = True
                for expectation in expectations:
                    exp_result = validate_expectation(actual_parsed, expectation)
            ...
            logger.error(f"Error running test case {test_id}, run {run_idx + 1}: {e}")
...
def run_all_evaluations(
    ...
    for test_case in test_cases:
        case_result = run_evaluation_case(test_case, provider, providers_state, workflow_path, system_message)
