                ...
                if extract_error:
                    ...
                    try:
                        ...
                        if (
                            ...
                            and not complete_content.strip()
                        ):
                            hint = (
                                " The LLM returned empty content. If the "
                                "model has a thinking/reasoning mode "
                                "(e.g. Gemma4, QwQ), max_tokens may be "
                                "exhausted before the response is produced. "
                                "Try increasing max_tokens (e.g. "
                                "max_tokens=2048)."
                            )
                        logger.error(
                            f"Failed to parse/validate streaming response "
                            f"for {workflow_name}: {e}{hint}"
                        )
                        final_chunk = {
                            "workflow": workflow_name,
                            "created": created,
                            "finish_reason": "stop",
                            "parse_error": f"{e}{hint}",
                            "llm_response": complete_content,
                        }
...
async def create_workflow_handler(workflow: Dict[str, Any]):
    ...
    async def handler(request: dict):
        ...
        _t0 = time.monotonic()
        logger.debug(
            "[workflow:%s] calling LLM (timeout=%s, max_tokens=%s, t=%.3f)",
            workflow_name,
            timeout,
            max_tokens,
            _t0,
        ...
        try:
            ...
            response = await call_llm_by_model(**llm_kwargs)
            logger.debug(
                "[workflow:%s] LLM returned in %.3fs",
                workflow_name,
                time.monotonic() - _t0,
            )
        except litellm.RateLimitError as e:
        ...
        try:
            ...
            if not original_content.strip():
                hint = (
                    " The LLM returned empty content. If the model has a "
                    "thinking/reasoning mode (e.g. Gemma4, QwQ), max_tokens "
                    "may be exhausted before the response is produced. "
                    "Try increasing max_tokens (e.g. max_tokens=2048)."
                )
            logger.error(
                f"Failed to parse JSON response for workflow "
                f"{workflow_name}: {e}{hint}"
            )
            logger.error(f"Raw content: {original_content!r}")
            raise HTTPException(
                status_code=400,
                detail={
                    "error": f"LLM returned invalid JSON: {str(e)}{hint}",
                    "llm_response": original_content,
                },
            )
