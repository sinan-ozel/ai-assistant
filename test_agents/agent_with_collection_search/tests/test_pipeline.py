"""Tests that both library shelves are ingested by the chunking pipeline.

The library contains two Markdown files placed directly (no PDF conversion
needed). Both are expected to be chunked and listed by /private/v1/books.
The file_path keys reported by the API use a .pdf extension even though the
sources are .md files (the chunking pipeline normalises the extension).
"""
...
# Allow enough time for chunking and embedding to complete.
CHUNK_TIMEOUT = 300  # seconds

# Expected paths as reported by /private/v1/books (.md → .pdf)
EXPECTED_BOOKS = {
    "shelf1/city-guide.pdf",
    "shelf2/city-guide.pdf",
}
