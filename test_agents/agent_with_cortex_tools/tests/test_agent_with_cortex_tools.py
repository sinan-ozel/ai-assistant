...
def test_mcp_server_lists_cortex_tools():
    ...
    assert "time_tool__get_current_time" in names, (
        f"Expected 'time_tool__get_current_time' in tool list; got: {names}"
    )
    assert "web_search__web_search" in names, (
        f"Expected 'web_search__web_search' in tool list; got: {names}"
    )
...
def test_mcp_get_current_time_tool_call():
    ...
    response = requests.post(
        ...
        json={
            ...
            "method": "tools/call",
            "params": {
                "name": "time_tool__get_current_time",
...
def test_mcp_get_current_time_annotations():
    """time_tool__get_current_time must declare readOnlyHint=True."""
    response = requests.post(
        f"{MCP_URL}/mcp",
    ...
    assert "time_tool__get_current_time" in tools
    annotations = tools["time_tool__get_current_time"]["annotations"]
    assert annotations.get("title") == "Get Current Time"
    assert annotations.get("readOnlyHint") is True, (
        "time_tool__get_current_time should declare readOnlyHint=True"
    )


@pytest.mark.depends(on=["mcp_lists_cortex_tools"])
def test_mcp_web_search_annotations():
    """web_search__web_search must declare readOnlyHint=True and openWorldHint=True."""
    response = requests.post(
        f"{MCP_URL}/mcp",
    ...
    assert response.status_code == 200
    tools = {t["name"]: t for t in response.json()["result"]["tools"]}
    assert "web_search__web_search" in tools
    annotations = tools["web_search__web_search"]["annotations"]
    assert annotations.get("title") == "Web Search"
    assert annotations.get("readOnlyHint") is True, (
        "web_search__web_search should declare readOnlyHint=True"
    )
    assert annotations.get("openWorldHint") is True, (
        "web_search__web_search should declare openWorldHint=True (calls external API)"
    )
