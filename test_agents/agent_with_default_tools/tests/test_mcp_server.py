...
def test_mcp_server_lists_search_tool():
    ...
    assert "search.library_search" in names, (
        f"Expected 'search.library_search' in tool list; got: {names}"
    )
...
def test_mcp_server_search_tool_schema():
    ...
    schema = tools_by_name["search.library_search"]

    assert schema.get("description"), "search.library_search must have a non-empty description"

    props = schema["inputSchema"]["properties"]
    for param in ("query", "collection", "top_k", "book"):
        assert param in props, (
            f"Expected parameter '{param}' in search.library_search inputSchema; "
            f"got: {list(props)}"
        )
